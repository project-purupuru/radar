...
- IDL is currently a DRAFT hand-derived from substrate Rust source. Real `anchor build` output should replace it before demo recording — see `src/idl/PROVENANCE.md` for replacement procedure. **2026-05-10 update**: A4 hard gate CLOSED — three real `StoneClaimed` events parsed via live EventParser on devnet; Candidate A shape (element/weather as `u8`/number, wallet/mint as PublicKey objects) verified to match radar's adapter exactly. DRAFT IDL is correct against the deployed program. Replacement still desirable for safety but no longer blocking.
- Discriminator computation in DRAFT IDL assumes default Anchor convention (`sha256("event:StoneClaimed")`). If substrate program uses `#[event(discriminator = "...")]` override, our IDL is wrong. Verify via `scripts/verify-parser-shape.ts` (sprint task A4). **2026-05-10**: discriminator `[138,131,241,101,8,187,119,216]` confirmed correct (three live events matched and parsed).
- **G5 finding · silent WS-only death is NOT caught by the dead-man timer**. Observed 2026-05-10 by deliberately setting `SOLANA_WS_URL=wss://invalid.example.com` while leaving `SOLANA_RPC_URL` valid. radar reported `connected: true` for the entire ~40s test window because `getSlot` (the dead-man trigger) uses HTTPS, not WS — so the heartbeat saw advancing slots even though the WS subscription was permanently broken. Operator signal DOES exist: `rpc-websockets` emits `ws error: getaddrinfo ENOTFOUND <host>` to stderr at ~1/sec, visible in Railway Deploy Logs. Operator response: combine `/health` polling with periodic `grep "ws error" railway-logs`. Post-hackathon remediation options (in order of robustness): (a) hook `Connection._rpcWebSocket.on('error', …)` in `src/client.ts` to surface a `wsErrors: number` counter on `/health`; (b) add a control subscription to a known-busy program (e.g., SPL Token Program) and treat absence of callbacks for ≥N min as WS-dead trigger; (c) periodically tear down + re-subscribe regardless of heartbeat state as a brute-force liveness probe. Of these, (a) is the simplest signal improvement, (b) is the proper architectural fix.
